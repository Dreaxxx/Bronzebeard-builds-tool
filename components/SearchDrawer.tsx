...
export default function SearchDrawer({
  ...
  const [ilvlMin] = useState<string>("");
  const [ilvlMax] = useState<string>("");
  ...
  const placeholder =
    kind === "items" ? t("search.placeholder.items") : t("search.placeholder.enchants");
  return (
    <Card>
        ...
        <div className="mt-3">
          ...
          {advOpen && (
            <div className="mt-2 grid gap-2 md:grid-cols-4">
              ...
              <input
                className="input"
                placeholder="Slot contains… (ex: Head)"
                value={slot}
          ...
          results.map((r) => (
            ...
            >
              <div className="text-sm">
                ...
                {r.quality && <span className="badge ml-2">{r.quality}</span>}
                {typeof r.ilvl === "number" && <span className="badge ml-2">ilvl {r.ilvl}</span>}
                {r.slotText && <span className="badge ml-2">{r.slotText}</span>}
              </div>
              <div className="flex items-center gap-2">
                <a className="btn" href={r.href} target="_blank" rel="noreferrer">
                  {t("search.openDb")}
                </a>
                <button className="btn" onClick={() => onPick(r)}>
                  {t("search.add")}
                </button>
              </div>
            </div>
