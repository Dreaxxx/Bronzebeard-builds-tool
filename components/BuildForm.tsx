    ...
    <Card>
      <div className="space-y-4">
        <div>
          ...
          <Input id="title" placeholder="Twilight Parangon Priest" value={title} onChange={e => setTitle(e.target.value)} />
        </div>
        <div>
        ...
        <div>
          ...
          <p className="text-xs text-neutral-500 mt-1">Ex: Raid,M0,M+&lt;10,M+10-14,M+15+</p>
        </div>
        <div className="space-y-2">
          <Label htmlFor="vis">{t('form.visibility')}</Label>
          <div className="flex items-center gap-4 text-sm">
            <label className="flex items-center gap-2">
              <input id="vis" type="radio" name="vis" checked={isPublic} onChange={() => setIsPublic(true)} />
              <span>{t('common.public')} (list in Explorer)</span>
            </label>
            <label htmlFor="vis-private" className="flex items-center gap-2">
              <input id="vis-private" type="radio" name="vis" checked={!isPublic} onChange={() => setIsPublic(false)} />
        ...
        <div className="space-y-1">
          <label htmlFor="comments" className="flex items-center gap-2 text-sm">
            <input id="comments" type="checkbox" disabled={!isPublic} checked={commentsEnabled} onChange={e => setCommentsEnabled(e.target.checked)} />
        ...
        <Row>
          <PrimaryButton onClick={() => {
            ...
            onSubmit({
              title, realm, role, classTag: classTag || undefined, tiers,
