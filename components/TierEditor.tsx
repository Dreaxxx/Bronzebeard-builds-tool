...
export default function TierEditor({ build }: { build: Build }) {
  ...
  useEffect(() => {
    (async () => setItems(await listItems(build.id, tier)))();
  }, [build.id, tier]);

  async function addItem(slot: Slot) {
    const name = prompt(`Item name for ${slot}?`);
    if (!name) return;
    await upsertItem({ buildId: build.id, slot, tier, name, rank: 1 });
    setItems(await listItems(build.id, tier));
  }
  async function saveItem(it: BuildItem) {
    await upsertItem(it);
    setItems(await listItems(build.id, tier));
  }
          ...
          <Card key={slot}>
              ...
              {(bySlot[slot] || []).map((it) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Input
                      value={it.name}
                      onChange={(e) => saveItem({ ...it, name: e.target.value })}
                    />
                    <div className="w-20">
                      <Label className="sr-only">Rank</Label>
                      <Input
                        type="number"
                        min={1}
                        value={it.rank}
                        onChange={(e) => saveItem({ ...it, rank: Number(e.target.value) })}
                      />
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Input
                      placeholder="Source (dungeon/raid/worldforged/craft)"
                      value={it.source ?? ""}
                      onChange={(e) => saveItem({ ...it, source: e.target.value })}
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    <Input
                    ...
                    />
                    {it.href && (
                      <a className="btn" href={it.href} target="_blank" rel="noreferrer">
                        Open
                      </a>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <Textarea
                      placeholder='Notes or stats (ex: {"SP":52,"Haste":28})'
                      value={it.notes ?? ""}
                      onChange={(e) => saveItem({ ...it, notes: e.target.value })}
                    />
                  </div>
                  {it.href && (
                    <a
            ...
            {openSlot === slot && (
              <div className="mt-3">
                ...
                />
                <div className="mt-2">
                  <button className="btn" onClick={() => setOpenSlot(null)}>
                    Close
                  </button>
                </div>
              </div>
