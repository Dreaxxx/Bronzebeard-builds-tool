...
export default function TierEditor({ build }: { build: Build }) {
  ...
  const bySlot = useMemo(() => { const m: Record<string, BuildItem[]> = {}; (SLOTS).forEach((s: string) => m[s] = []); for (const it of items) { (m[it.slot] = m[it.slot] || []).push(it); } Object.keys(m).forEach(s => m[s].sort((a, b) => a.rank - b.rank)); return m; }, [items]);
