...
export default function Explore() {
  ...
  const classes = useMemo(
    () => Array.from(new Set(builds.map((b) => b.classTag).filter(Boolean))) as string[],
    [builds],
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {sorted.length === 0 && (
        <Card>
          <p className="text-sm text-neutral-500">No public builds yet.</p>
        </Card>
      )}
        ...
        {sorted.map((b) => (
          <Card key={b.id}>
            <div className="text-sm text-neutral-500">
              {b.realm} • {b.role}
              {b.classTag ? ` • ${b.classTag}` : ""}
            </div>
            <h3 className="text-lg font-semibold">{b.title}</h3>
            <div className="mt-1 text-xs text-neutral-500">
              ❤️ {b.likes || 0} • {t("common.created")} {new Date(b.createdAt).toLocaleDateString()}{" "}
              • {t("common.updated")} {new Date(b.updatedAt).toLocaleDateString()}
            </div>
            <div className="mt-3 flex gap-2">
              <Link className="btn bg-green-500 hover:bg-green-600" href={`/builds/${b.id}/view`}>
                {t("common.view")}
              </Link>
            </div>
          </Card>
