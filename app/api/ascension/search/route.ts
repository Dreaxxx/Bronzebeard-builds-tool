...
type Res = { id: string; type: 'item'|'spell'; name: string; href: string; quality?: string; ilvl?: number; slotText?: string };
...
function parseNearby(text: string): { ilvl?: number; slotText?: string } {
  ...
  const slot = /(Head|Neck|Shoulder|Back|Chest|Wrist|Hands|Waist|Legs|Feet|Ring|Trinket|Weapon|Off[- ]?hand|Ranged|Tabard)/i.exec(text);
...
function parseResults(html: string): Res[]{
  ...
  while ((m = re.exec(html))) {
    const full = m[1]; const id = m[2]; const name = m[3].trim();
    ...
    if (name && id && !results.find(r=>r.id===id && r.type==='item')) results.push({ id, type:'item', name, href, quality, ilvl: ctx.ilvl, slotText: ctx.slotText });
  }
  const spellRe = /href="\/(?:\?spell=(\d+)[^"]*)"[^>]*>([^<]+)<\/a>/g;
  let s: RegExpExecArray | null;
  while ((s = spellRe.exec(html))) {
    const id = s[1]; const name = s[2].trim();
    const href = `https://db.ascension.gg/?spell=${id}`;
    if (name && id && !results.find(r=>r.id===id && r.type==='spell')) results.push({ id, type:'spell', name, href });
  }
...
export async function GET(req: NextRequest){
  ...
  const quality = (searchParams.get('quality')||'').trim(); // Uncommon|Rare|Epic|Legendary
