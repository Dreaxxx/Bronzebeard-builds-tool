...
export async function uploadBuild(build: Build, items: BuildItem[], enchants: Enchant[]){
  ...
  if(enchants.length){ const payload=enchants.map(en=>({ id:en.id, build_id:build.id, name:en.name, rarity:en.rarity, slot:en.slot, tags:en.tags, cost:en.cost??null, notes:en.notes??null, href: en.href ?? null })); const {error:e3}=await sb.from("build_enchants").insert(payload); if(e3) throw e3; }
}
